function _1(md){return(
  md`<div style="color: grey; font: 30px/25.5px var(--sans-serif); text-transform: uppercase;"><h1 style="display: none;">MotoGP Ranking</h1></div>
  
  # MotorGP
  
  This animated bar chart shows the MotoGP rankings since 2003. I have mixed in the points of the motorcycle manufacturers as well. Data source: [MotoGP official website](https://www.motogp.com/en/world-standing/2024/motogp/championship-standings)
  `
)}
    //const prioritizedItems = ["HONDA", "YAMAHA", "SUZUKI", "KAWASAKI", "DUCATI", "APRILIA", "KR211V", "ART", "FTR", "FTR KAWASAKI", "FORWARD YAMAHA", "KTM"]; // 将需要优先显示的条目名称放在这里
    const prioritizedItems = [];
    const specialColors = {
        "HONDA": "#32CD32",       // Lime Green
        "YAMAHA": "#32CD32",      // Lime Green
        "SUZUKI": "#32CD32",      // Lime Green
        "KAWASAKI": "#32CD32",    // Lime Green
        "DUCATI": "#32CD32",      // Lime Green
        "APRILIA": "#32CD32",     // Lime Green
        "KR211V": "#32CD32",      // Lime Green
        "ART": "#32CD32",         // Lime Green
        "FTR": "#32CD32",         // Lime Green
        "FTR KAWASAKI": "#32CD32",// Lime Green
        "FORWARD YAMAHA": "#32CD32", // Lime Green
        "KTM": "#32CD32"          // Lime Green
      };
    ...
    function rank(value) {
      const data = Array.from(names, name => ({
        name,
        value: value(name),
        priority: prioritizedItems.includes(name) ? 1 : 0 // 为特定条目添加优先级
      }));
      
      // 首先按优先级排序，然后按值排序
      data.sort((a, b) => d3.descending(a.priority, b.priority) || d3.descending(a.value, b.value));
    ...
    function _color(d3,data, specialColors)
    {
      const scale = d3.scaleOrdinal(d3.schemeTableau10);
      if (data.some(d => d.category !== undefined)) {
        const categoryByName = new Map(data.map(d => [d.name, d.category]))
        scale.domain(categoryByName.values());
        return d => specialColors[d.name] || scale(categoryByName.get(d.name));
      }
      return d => specialColors[d.name] || scale(d.name);
    }
    ...
    export default function define(runtime, observer) {
        ...
        const fileAttachments = new Map([
          ["category-brands.csv", {url: new URL("./files/motogp.csv", import.meta.url), mimeType: "text/csv", toString}]
        ]);
        ...
        main.variable(observer("color")).define("color", ["d3","data","specialColors"], _color);
        ...
        main.variable(observer("specialColors")).define("specialColors", _ => specialColors);
        return main;
      }
